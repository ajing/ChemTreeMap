...
angular.module('frontendApp')
    .factory('dataService', function ($http, $q) {
      ...
      var flatten_dict = function (root) {
        var nodes = {}, i = 0;
      ...
      var linkFunc = function(d) {  return linkRange(Number(d.target.dist)); };
      ...
      dataService.loadExample = function(name, callback) {
        ...
        $http.get('data/' + name + '.json')
            .then(function(response) {
              ...
              var root  = response.data.trees[response.data.metadata.treeTypes[0].name],
                  nodes = dataService.flatten(root),
                  links = d3.layout.tree().links(nodes);

              var forces = {},
                  i = 0;
              for (i = 0; i <  response.data.metadata.treeTypes.length; i++) {
                var force = d3.layout.force()
                  .charge(function(d) { return d._children ? -d.size * 100 : -50; })
                  .linkDistance(linkFunc)
               //   .size([1,1]);
                ...
                root  = response.data.trees[response.data.metadata.treeTypes[i].name];
                nodes = dataService.flatten(root);
                links = d3.layout.tree().links(nodes);
                ...
                forces[response.data.metadata.treeTypes[i].name] = force;

              }

              dataService.data.forces= forces;
              dataService.data.force = forces[response.data.metadata.treeTypes[0].name];
